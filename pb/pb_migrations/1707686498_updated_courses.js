...
migrate((db) => {
  ...
  collection.listRule = "@request.auth.id != \"\" && assignees.id ?= @request.auth.id && id ?= @collection.lessons.course.id"
  collection.viewRule = "@request.auth.id != \"\" && assignees.id ?= @request.auth.id && id ?= @collection.lessons.course.id"
